...
<div class=" w-full flex-col items-center justify-center px-7 py-5 text-center">
	<h2 class="tracking-tight-md animate-fade-in-scale text-[1.33rem] leading-loose font-[550]">
		{#if data.authTitle}{data.authTitle}{:else if $emailMessage?.existingUser}Welcome back to Luxo{:else}Welcome
			to Luxo{/if}
	</h2>
	{#if data.authMessage}
		<p
		...
		<ToastSwap bind:trigger={toastConfirmCopied}>
			{#snippet swapContent()}
				<div in:wipeHorizontal={{ duration: 400 }} class="flex h-7 items-center">
					<p class="w-fit rounded-full bg-green-500 px-2 text-[0.93rem] text-white">
						Copied to Clipboard
					</p>
				</div>
			{/snippet}
			<p
				class="flex h-7 items-center text-neutral-300 transition-colors duration-100 group-hover:text-white"
			>
				Click to Copy
			</p>
		</ToastSwap>
